...
import { File, MessageCircle, Send, Upload, X } from "lucide-react";
    ...
    const initializeChat = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const existingMessages: Message[] = conversation.messages.map(
            (msg: any) => ({
              id: msg.id,
              role: msg.role,
              content: msg.content,
          ...
          if (conversation.documents?.length > 0) {
            const seen = new Set();
            const uniqueDocuments = conversation.documents.filter(
              (doc: any) => {
            ...
            const fileObjects: UploadedFile[] = uniqueDocuments.map(
              (doc: any) => ({
                name: doc.name,
                size: doc.size,
              }),
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.documents?.length > 0) {
          ...
          await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/api/conversations/${conversationId}/documents`,
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
            ...
            <div className="bg-slate-800 rounded-xl shadow-lg border border-slate-700 h-[600px] flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                ...
                {messages.map((message) => (
                  ...
                  >
                    <div
                      className={`max-w-[80%] rounded-xl px-4 py-3 ${getMessageStyles(message.role)}`}
                    >
                      {message.role === "AI" ? (
